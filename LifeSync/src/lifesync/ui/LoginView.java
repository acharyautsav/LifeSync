...
public class LoginView extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label signUpQuestion = new Label("Don't have an account?");
        signUpQuestion.setStyle("-fx-font-size: 16px; -fx-font-family: 'Poppins'; -fx-text-fill: #3F3D56;");

        // Sign-up button with refined hover effects
        Button signUpButton = createStyledButton("Sign Up");
        addButtonHoverEffect(signUpButton);
        signUpButton.setOnAction(e -> new RegisterView().start(primaryStage));

        // Adding elements to the sidebar
        sidebar.getChildren().addAll(welcomeLabel, signUpQuestion, signUpButton);
